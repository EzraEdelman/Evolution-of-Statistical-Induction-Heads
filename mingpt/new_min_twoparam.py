...
class min_model(nn.Module):
    ...
    def __init__(self, config):
        ...
        with torch.no_grad():
            # self.v.weight[0] = 1
            self.Wq.weight = torch.nn.Parameter(torch.eye(num_tokens)*4.6)
    ...
    def forward(self, idx, targets=None):
        ...
        pos_embd = torch.zeros_like(self.pos).squeeze()
        pos_embd[1:].fill_diagonal_(1)
        pos_embd[0,0]=1

        # l1 = pos_embd.tril() @ e
        attn = pos_embd * self.v.weight[0]
        ...
        # attn = (self.Wq.weight[0,0] * e @ l1.transpose(-2,-1))
        attn = (self.Wq.weight[0,0] * e @ l1.transpose(-2,-1))
        attn = attn.masked_fill(self.bias, float('-inf'))
        attn = F.softmax(attn, dim=-1)
        # attn = self.softmax_approx(attn)
        l2 = attn @ e * 3.1#* 3.1#self.Wv.weight[0,0]
