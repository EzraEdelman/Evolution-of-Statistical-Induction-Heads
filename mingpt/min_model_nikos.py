...
class min_model(nn.Module):
    ...
    def __init__(self, config):
        ...
        # with torch.no_grad():
        #     self.v.weight = nn.Parameter(torch.zeros((self.length,1)))
        #     self.v.weight[1,0]=1
        #     self.Wk.weight = nn.Parameter(torch.eye(self.num_tokens))
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            # mask = targets != -1
            # temp_logits = logits.view(-1, logits.size(-1))[mask.view(-1)]
            # targets = targets.view(-1)[mask.view(-1)]
            # loss = F.multi_margin_loss(logits.view(-1, logits.size(-1)), targets.view(-1), margin=float("inf"))
