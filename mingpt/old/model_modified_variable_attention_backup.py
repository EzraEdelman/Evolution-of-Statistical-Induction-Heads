...
class AssociationAttention(nn.Module):
    ...
    def forward(self, x, idx):
        ...
        #have every token x_t attend to tokens that match the token before them (x_{t-1})
        att = self.raw_attention(x, idx)
        

        att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
        #put negative weights to emphasize where we are attending to, can also just multiple att by some constant
...
class FixedAttention(nn.Module):
    ...
    def __init__(self, config, raw_attention):
        super().__init__()
        assert config.n_embd % config.n_head == 0
        # value projections for all heads, but in a batch
        self.c_attn = nn.Linear(config.n_embd, config.n_embd)
...
class Attention_Only_GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert config.model_type in ["gpt", "first-layer", "second-layer"], 
        if config.model_type == "gpt":
            layers = [Block(config) for _ in range(config.n_layer)]
        elif config.model_type == "first-layer":
            layers = [Block(config), FixedBlock(config, AssociationAttention)]
        elif config.model_type == "second-layer":
            layers = [FixedBlock(config, FixedAttention), Block(config)]
        else:
            assert False, f"{config.model_type} not a valid model type"
