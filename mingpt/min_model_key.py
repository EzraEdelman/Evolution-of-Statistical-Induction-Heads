...
class min_model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.v = nn.Parameter(torch.zeros((self.length,1)))
        ...
        with torch.no_grad():
            ...
            self.v[1] += .1
            self.W.weight += nn.Parameter(torch.eye(self.num_tokens))
    ...
    def forward(self, idx, targets=None):
        ...
        layer_one = (e @ torch.ones((self.length, self.length), device=idx.device).tril(diagonal=0))*self.v

        # (M @ e) @ W
        attention = e @ layer_one.transpose(1,2)#self.W(layer_one).transpose(1, 2)
    ...
    def configure_optimizers(self, train_config):
        ...
        optim_groups.append({"params": [self.v], "weight_decay": 0})
