...
class Attention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # relative embeddings
        self.wpe_k = nn.Embedding(config.block_size + 1, config.n_embd, padding_idx = config.block_size)
        self.wpe_v = nn.Embedding(config.block_size + 1, config.n_embd, padding_idx = config.block_size)
    ...
    def forward(self, x):
        ...
        rel_pos_k = self.embd_dropout(self.wpe_k(self.pos[:T,:T]))
        rel_pos_k = rel_pos_k.view(T, T, self.n_head, C // self.n_head) # (T, T, nh, hs)
        rel_pos_v = self.embd_dropout(self.wpe_v(self.pos[:T,:T]))
        rel_pos_v = rel_pos_v.view(T, T, self.n_head, C // self.n_head) # (T, T, nh, hs)

        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
        attn = (q @ k.transpose(-2, -1) + torch.einsum("Tthe,BhTe->BhTt", rel_pos_k, q)) * (1.0 / math.sqrt(k.size(-1)))
        ...
        # temp = attn @ rel_pos_v
        # temp = temp.transpose(0, 1).contiguous().view(B, self.n_head, T, C // self.n_head)
        temp = torch.einsum("BhTt,Tths->BhTs", attn, rel_pos_v)
        # print(B, self.n_head, T, C // self.n_head)
        # temp = temp.transpose(0, 1).contiguous().view(B, self.n_head, T, C // self.n_head)
        y = y + temp
